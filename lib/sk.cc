...
none sk_resize_texture(sk a, i32 w, i32 h) {
    resize(a->tx, w, h);
    sk_dealloc(a);
    sk_init(a);
}
...
none sk_init(sk a) {
    ...
    GrVkImageInfo info       = {};
    info.fImage              = (VkImage)a->tx->_vk_image;
...
none sk_output_mode(sk a, bool output) {
    ...
    //transition(a->tx, to);
    transition_image_layout(a->t, (VkImage)a->tx->_vk_image, fr, to,
        0, 1, 0, 1, false);
    a->tx->_vk_layout = (i32)to;
}
